...
extension Archive {
    ...
    func readCompressed(entry: Entry, bufferSize: Int, skipCRC32: Bool, useZlib: Bool = false,
        ...
        guard size <= .max else { throw ArchiveError.invalidEntrySize }
        if useZlib {
            // Take the bound from the central directory, like `size` above. A ZIP64-format
            // data descriptor behind a non-ZIP64 central record is otherwise misread as 32-bit.
            let expectedSize = entry.centralDirectoryStructure.effectiveUncompressedSize
            return try Data.inflateImport(size: size, expectedSize: expectedSize,
