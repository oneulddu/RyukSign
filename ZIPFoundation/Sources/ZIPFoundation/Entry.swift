...
extension Entry.CentralDirectoryStructure {
    ...
    var effectiveCompressedSize: UInt64 {
        if self.isZIP64, let compressedSize = self.zip64ExtendedInformation?.compressedSize, (compressedSize > 0 || self.compressedSize == .max) {
            return compressedSize
        }
        return UInt64(compressedSize)
    }
    var effectiveUncompressedSize: UInt64 {
        if self.isZIP64, let uncompressedSize = self.zip64ExtendedInformation?.uncompressedSize, (uncompressedSize > 0 || self.uncompressedSize == .max) {
            return uncompressedSize
        }
        return UInt64(uncompressedSize)
    }
    var effectiveRelativeOffsetOfLocalHeader: UInt64 {
        if self.isZIP64, let offset = self.zip64ExtendedInformation?.relativeOffsetOfLocalHeader, (offset > 0 || self.relativeOffsetOfLocalHeader == .max) {
            return offset
        }
